...
public class SchemaService
{
    ...
    private async Task CreateProjectSchemaInternalAsync(string schemaName)
    {
        ...
        var createBadFileReportTableQuery = $@"
            CREATE TABLE IF NOT EXISTS ""{schemaName}"".""BadFileReport"" (
                ...
                CONSTRAINT ""FK_{schemaName}_BadFileReport_ResolvedBy"" FOREIGN KEY (""ResolvedBy"") REFERENCES ""dbo"".""ApplicationUser"" (""UserId"") ON DELETE NO ACTION
            );
        ...
        // Migrate existing BadFileReport tables to add new columns (PostgreSQL ADD COLUMN IF NOT EXISTS)
    ...
    public async Task<bool> SchemaExistsAsync(string schemaName)
    {
        try
        {
            var sql = @"SELECT COUNT(*)::int FROM information_schema.schemata WHERE schema_name = @p0";
    ...
    public async Task MigrateExistingSchemaAsync(string schemaName)
    {
        ...
        // Migrate Document table - PostgreSQL ADD COLUMN IF NOT EXISTS
        var migrateDocumentQuery = $@"
            ...
            -- Add unique filtered index on PublicToken
    ...
    public async Task MigrateAllExistingSchemasAsync()
    {
        ...
        var getSchemasQuery = @"
            ...
            AND t.table_schema NOT IN ('dbo', 'public', 'pg_catalog', 'information_schema')
